...
class TYTiledRender: NSObject {
    class func rendImage(withImageSource path: String, show view: UIView ,frame: CGRect) {
        
        // Do any additional setup after loading the view, typically from a nib.
        var scrollView_frame = CGRect(x: CGFloat(0), y: CGFloat(0), width: CGFloat(view.bounds.size.width), height: CGFloat(view.bounds.size.height))
        if frame.width != 0 && frame.height != 0 { // cus Frame
            scrollView_frame = frame
        }
        //Bitmap
