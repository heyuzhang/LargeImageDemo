...
//    }
    ...
    enum  ComposeImageDirection{
        case leftAndRight
        case upAndDown
    }
    
        //拼接图片
    func composeImage(image1:UIImage ,toImage image2:UIImage, according direction: ComposeImageDirection) -> UIImage? {
        
        let newImageW: CGFloat
        let newImageH: CGFloat
        
        switch direction {
        case .leftAndRight://左右
            
            newImageW = image1.size.width + image2.size.width
            newImageH = max(image1.size.height, image2.size.height)
            
            break
        case .upAndDown: //上下
            
            newImageW = max(image1.size.width, image2.size.width)
            newImageH = image1.size.height + image2.size.height
            break
        }
        
        let newImageSize = CGSize(width: newImageW, height: newImageH)
        
        UIGraphicsBeginImageContextWithOptions(newImageSize, false, UIScreen.main.scale);
        
        
        var image1Rect: CGRect = .zero
        var image2Rect: CGRect = .zero
        ...
        case .leftAndRight://左右
             ...
             image1Rect = CGRect(x: 0, y: 0, width: image1.size.width, height: newImageH)
             image1.draw(in: image1Rect)
            image2Rect = CGRect(x: image1Rect.size.width, y: 0, width: image2.size.width, height: newImageH)
        ...
        case .upAndDown: //上下
            ...
            image1Rect = CGRect(x: 0, y: 0, width: newImageW, height: image1Rect.size.height)

            
            image2Rect = CGRect(x: 0, y: image1Rect.size.height, width: newImageW, height: image2Rect.size.height)
            image2.draw(in: image2Rect)
            break
        }
