...
//    var numberOfZoomLevels = size_t()
    var cgSourceImage: CGImage? = nil {
        didSet {
            ...
            let midImage = setImageMid(maxLength: 100 * 1024 * 1024, image: image) // 10M
...
extension TYTiledView {
    
    func setImageMid(maxLength: Int , image: UIImage) -> UIImage? {
        
        guard let vData = UIImageJPEGRepresentation(image, 1) else { return nil }
        print("压缩前kb: \( Double((vData.count)/1024))")
        if vData.count < maxLength {
            return UIImage(data: vData)
        }
        var compress: CGFloat = 0.4
        guard var data = UIImageJPEGRepresentation(image, compress) else { return nil }
